...
Paint Panic es una experiencia interactiva desarrollada en p5.js que combina técnicas de visión artificial con principios de diseño generativo. El proyecto integra dos formas de percepción del usuario frente a la cámara — detección de color y detección de movimiento por frame difference — para alimentar el comportamiento de un personaje autónomo que pinta, reacciona y "entra en pánico" según lo que ve.

El nombre del proyecto describe su naturaleza: un sistema que vive en el límite entre el control del usuario y el caos generativo propio. El usuario pinta con las manos usando marcadores de color frente a la cámara; en paralelo, un personaje con personalidad propia observa el movimiento corporal del usuario y reacciona con sus propios estados de ánimo, pinceladas y "travesuras" visuales — de ahí el pánico: nunca se sabe del todo qué va a hacer a continuación.
...
Existe un agente autónomo cuyo comportamiento está basado en una máquina de estados finitos, modulada por una personalidad aleatoria y por dos capas de percepción: **color + movimiento**. Cada instancia nace con rasgos propios —energía, curiosidad, terquedad, travesura y dramatismo— que determinan cómo se comporta y hacen que ningún personaje sea exactamente igual a otro.

### Estados de comportamiento

* **Vagar** — deambula con movimiento orgánico generado mediante ruido Perlin y deja un rastro tenue de su color.
...
* **Cada gesto nuevo detectado** → puede provocar inmediatamente una travesura, como una bomba de color o un cambio de filtro.
...
Las bombas pueden utilizar diferentes patrones geométricos —spray, anillo o línea— y distintos tamaños —mediano, grande o extragrande—. Su color puede ser **multicolor y armónico**, basado en la paleta HSB del personaje, o **monocromático**, utilizando diferentes niveles de opacidad.
